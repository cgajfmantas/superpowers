...
**Put in the skill only what transfers; leave the evidence in the report.** Phase 2 edits a skill minutes after reading the measurements, which is exactly when they feel most persuasive — and the numbers that convinced you are this run's, not the reader's. Test each one: *would this help someone reading the skill in a different project?* Byte counts of files and how often something was restated transfer; the names of one codebase's defect classes do not. Give a rule the single most general fact that stops it reading as arbitrary, and leave the breakdown in the report, where it is already written down. Observed three times during this skill's own first use — dollar amounts twice and a defect-class list once, each time reasoned as "measured evidence, so it belongs".
