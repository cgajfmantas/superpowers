...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **Spec for reference:** [SPEC_FILE_PATH]
    ...
    | Base Commit | Index header records a Base commit, as a full SHA. Missing is an issue: it is the tree every transcribed signature, `emits` list and measured value in the plan was read from, and without it nobody can tell whether those transcriptions still describe the code |
    | Value Provenance | Every exact value in a constraint file or task — measurement, default, threshold, magic number — either traces to a named source (spec, tool docs, code at the Base commit) or carries the command that produced it. A value reasoned from a framework's defaults is an issue. A number printed beside an instruction to measure that same thing is an issue: the number is what implementers use |
    | Command State | Each command the plan prescribes says which state it reads — staged index, working tree, commit range, built artifact, running service — and where it runs relative to that state. An expected output that a run inspecting *nothing* would also produce is an issue: a gate over staged files run after commit passes vacuously |
    | Test-Case Reachability | For each task, every declared test case can pass by changing only the files in that task's Files list — judged from the plan's own text, not by crawling the codebase. A component or module named in the task's title, Behavior, Interfaces or test cases but absent from its Files list is an issue: the implementer hits a contradiction inside its own brief and either stops or ships the listed files with the declared test failing |
