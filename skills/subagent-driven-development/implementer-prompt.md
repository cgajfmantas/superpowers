...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **While you work:** If you encounter something unexpected or unclear, pause and report NEEDS_CONTEXT. Do not guess or make unsupported assumptions. Questions requiring human input go through the controller's ask tool.
    ...
    - If you must poll something yourself, never sleep more than 240 seconds per poll, and never try to sleep out the whole wait in one call. A longer pause expires the prompt cache and makes the next poll re-charge your entire context at the write rate — the single most expensive mistake observed in this role. Every dispatched agent gets the short cache TTL, so this holds regardless of what the session around you is doing.
    - Launch a suite you are handing off with `setsid nohup`, not the harness's background-execution flag. The flag notifies whoever launched the job, and you are about to stop — so the notification would reach nobody, which is why the sentinel exists. (One run also lost two launches of a multi-hour suite under that flag for reasons never established; detaching keeps you out of that failure mode too.) If instead you are staying alive and waiting for something short, the background flag is the better tool: it notifies you on exit and costs no poll turns.
